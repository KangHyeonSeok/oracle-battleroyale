...
- UpdatedAt: 2026-04-05T22:27:42.899Z
- Detail: spec-validator: All four spec acceptance criteria passed per validation evidence. Gemini intent extraction, credulity judgment with override_queue integration, cooldown enforcement, and point management are all implemented and syntax-validated.
...
Status: passed
Evidence: routes.js returns HTTP 402 when points < 5; oracle/points.js deductOracleCost() uses atomic UPDATE ... WHERE constellation_points >= 5 RETURNING to prevent race conditions. Survival +1/turn, win +5, dramatic reversal +15 awarded from turn-scheduler.
