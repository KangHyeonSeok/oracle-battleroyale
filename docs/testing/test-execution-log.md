...
| load-32players | ✅ 통과 | p50=0.371ms, p95=3.539ms, p99=9.158ms, max=9.158ms (SLA 1000ms 이하) |
...
| 이슈 | 내용 |
|------|------|
| `berserk` 클래스 스펙 불일치 | `npc-presets.js`에 정의된 berserk 클래스가 공식 5종(warrior/archer/mage/assassin/healer)에 미포함. 정책 확인 필요 |
| healer FFA 동작 | FFA 환경에서 힐러가 가장 낮은 HP의 '적'을 치료하는 현상. 의도된 설계인지 확인 필요 |
| 실제 DB/Redis 연동 테스트 없음 | 모든 테스트가 인메모리 mock 사용. 실제 PostgreSQL·Redis 연결 통합 테스트 부재 |
| GEMINI_API_KEY 미설정 | 환경변수 미설정 시 keyword fallback 동작 확인됨. 실제 서비스 배포 전 API 키 설정 필요 |

---

## 2026-04-13 #21 (태연 스케줄 실행 — 21회차)
...
| e2e-flow | ✅ 7/7 단계 통과 | GEMINI_API_KEY 미설정 → keyword fallback 정상 동작 |
| matchmaker | ✅ 전체 통과 | 32플레이어 즉시 매칭, NPC 22명 충원, 중복/이탈 정상 |
| load-32players | ✅ 통과 | p50=0.379ms, p95=4.270ms, p99=5.293ms, max=5.293ms (SLA 1000ms 이하) |
| gemini-cost | ✅ 통과 | 최악 $0.001416 / 일반 $0.000442 (예산 $0.005 이하) |

### 수정한 버그

없음 — 이전 회차(#20) 수정 이후 전체 그린.
...
| e2e-flow | ✅ 7/7 단계 통과 | GEMINI_API_KEY 미설정 → keyword fallback 정상 동작 |
| matchmaker | ✅ 전체 통과 | 32플레이어 즉시 매칭, NPC 22명 충원, 중복/이탈 정상 |
| load-32players | ✅ 통과 | p50=0.383ms, p95=3.364ms, p99=5.947ms, max=5.947ms (SLA 1000ms 이하) |
| gemini-cost | ✅ 통과 | 최악 $0.001416 / 일반 $0.000442 (예산 $0.005 이하) |
...
| 파일 | 수정 내용 |
|------|-----------|
| `src/game/redis-state.js` | `buildInitialState`에서 `max_hp`, `attack`, `defense`, `speed` 필드 누락 수정. 전투 모듈이 폴백값(attack=10, defense=0)으로 처리되어 클래스 밸런스 무력화되던 문제 해소. HP 초기값도 `max_hp` 기준 full HP로 변경 |
| `src/game/matchmaker.js` | `_startMatch()` SELECT 쿼리에 `hp`, `max_hp`, `attack`, `defense`, `speed` 필드 추가. redis-state.js 수정과 함께 클래스 스탯이 실제로 전달되도록 연결 |
...
| e2e-flow | ✅ 7/7 단계 통과 | GEMINI_API_KEY 미설정 → keyword fallback 정상 동작 |
| matchmaker | ✅ 전체 통과 | 32플레이어 즉시 매칭, NPC 22명 충원, 중복/이탈 정상 |
| load-32players | ✅ 통과 | p50=0.316ms, p95=2.906ms, p99=6.581ms, max=6.581ms (SLA 1000ms 이하) |
| gemini-cost | ✅ 통과 | 최악 $0.001416 / 일반 $0.000442 (예산 $0.005 이하) |
...
| e2e-flow | ✅ 7/7 단계 통과 | GEMINI_API_KEY 미설정 → keyword fallback 정상 동작 |
| matchmaker | ✅ 전체 통과 | 32플레이어 즉시 매칭, NPC 22명 충원, 중복/이탈 정상 |
| load-32players | ✅ 통과 | p50=0.448ms, p95=2.755ms, p99=8.117ms, max=8.117ms (SLA 1000ms 이하) |
| gemini-cost | ✅ 통과 | 최악 $0.001416 / 일반 $0.000442 (예산 $0.005 이하) |
...
| e2e-flow | ✅ 7/7 단계 통과 | GEMINI_API_KEY 미설정 → keyword fallback 정상 동작 |
| matchmaker | ✅ 전체 통과 | 32플레이어 즉시 매칭, NPC 22명 충원, 중복/이탈 정상 |
| load-32players | ✅ 통과 | p50=0.324ms, p95=2.680ms, p99=5.013ms, max=5.013ms (SLA 1000ms 이하) |
| gemini-cost | ✅ 통과 | 최악 $0.001416 / 일반 $0.000442 (예산 $0.005 이하) |
...
| e2e-flow | ✅ 7/7 단계 통과 | GEMINI_API_KEY 미설정 → keyword fallback 정상 동작 |
| matchmaker | ✅ 전체 통과 | 32플레이어 즉시 매칭, NPC 22명 충원, 중복/이탈 정상 |
| load-32players | ✅ 통과 | p50=0.608ms, p95=4.543ms, p99=7.311ms, max=7.311ms (SLA 1000ms 이하) |
| gemini-cost | ✅ 통과 | 최악 $0.001416 / 일반 $0.000442 (예산 $0.005 이하) |
...
| e2e-flow | ✅ 7/7 단계 통과 | GEMINI_API_KEY 미설정 → keyword fallback 정상 동작 |
| matchmaker | ✅ 전체 통과 | 32플레이어 즉시 매칭, NPC 22명 충원, 중복/이탈 정상 |
| load-32players | ✅ 통과 | p50=0.377ms, p95=2.160ms, p99=6.283ms, max=6.283ms (SLA 1000ms 이하) |
| gemini-cost | ✅ 통과 | 최악 $0.001416 / 일반 $0.000442 (예산 $0.005 이하) |
...
## 2026-04-13 (태연 스케줄 2차 실행)

**실행 시각**: 2026-04-13  
**실행 환경**: Node.js, `/shared/oracle-battleroyale/server`  
**실행 방법**: `npm test` (package.json test 스크립트 정상 동작 확인)
...
| points | ✅ 18/18 통과 | oracle_send -10pt, win_bonus +50pt, 일일 로그인 중복 방지 정상 |
| e2e-flow | ✅ 7/7 단계 통과 | GEMINI_API_KEY 미설정 → keyword fallback 정상 동작 |
| matchmaker | ✅ 4/4 통과 | 32플레이어 즉시 매칭, NPC 22명 충원, 중복/이탈 정상 |
| load-32players | ✅ 통과 | p50=0.307ms, p95=2.571ms, p99=5.093ms, max=5.093ms (SLA 1000ms 이하) |
| gemini-cost | ✅ 통과 | 최악 $0.001416 / 일반 $0.000442 (예산 $0.005 이하) |
