...
| 구분 | 파일 | 실행 결과 | 비고 |
|------|------|-----------|------|
| 단위 — 전투 로직 | `test/combat.test.js` | ✅ 17/17 통과 | 20회차: redis-state.js·matchmaker.js 스탯 누락 버그 수정 후 재검증 |
| 단위 — 포인트 시스템 | `test/points.test.js` | ✅ 18/18 통과 | 커스텀 assert harness |
| 단위 — 매치메이킹 | `test/matchmaker.test.js` | ✅ 전체 통과 | Node.js assert |
| 통합 — E2E 플로우 | `test/e2e-flow.test.js` | ✅ 7단계 통과 | Module stub 기반, 실 DB/Redis 없음 |
| 부하 — 32명 동시 | `test/load-32players.test.js` | ✅ p99 ≈ 5.9ms | SLA 1,000ms 이하 유지 |
| 비용 검증 — Gemini | `test/gemini-cost.test.js` | ✅ $0.001416 | 예산 $0.005 이하 유지 |
| oracle-cooldown 단위 | `test/oracle-cooldown.test.js` | ❌ 부재 | **P1-4** — server-unit-tests-expansion queued, runner 착수 대기 |
| leaderboard 단위 | `test/leaderboard.test.js` | ❌ 부재 | **P1-5** — server-unit-tests-expansion queued, runner 착수 대기 |
| CI 서버 테스트 워크플로 | `.github/workflows/server-test.yml` | ❌ 미포함 | **P0-2 — 20회차 연속** (server-test-ci needs-review, hyeonseok 착수 승인 대기) |
| 실 WebSocket 통합 테스트 | — | ❌ 부재 | P1-1 미착수 |
| DB 마이그레이션 스모크 테스트 | `migrations/` (7개) | ❌ 부재 | P1-2 미착수 |
| Godot 클라이언트 / Playwright E2E | `tests/smoke.spec.js` | ⚠️ canvas 렌더만 검증 | playwright-debug-setup flow done (JavaScriptBridge 구현 완료), WS·OAuth·매치 플로우 미검증 |

### 발견한 문제점 (19회차 대비)

1. **P0-2 CI 워크플로 20회차 연속 미해결** — `server-test-ci` spec 완성(needs-review), push/PR 트리거·Node.js 20·Redis 7·PostgreSQL 15·AC 5개 명세 완비. hyeonseok 착수 승인 없이 CI 없는 라이브 배포 20회 연속 지속.
2. **server-unit-tests-expansion queued — runner 미시작** — `oracle-cooldown.test.js`(Redis mock 기반, 6케이스)·`leaderboard.test.js`(tiebreak/NULL rank/limit/displayName fallback, 8케이스) spec 완비·queued 전환 완료. runner 착수 대기 상태. 구현 완료 모듈에 테스트 없이 라이브 상태 지속.
3. **20회차 전투 버그 수정 완료 — 회귀 방지 체계 부재** — `redis-state.js`(max_hp·attack·defense·speed 필드 누락)·`matchmaker.js`(SELECT 쿼리 스탯 필드 미포함) 버그가 테스트 실행 중 발견·수정됨. CI 없는 환경에서 유사 버그 조기 감지 불가 구조 지속.
4. **playwright-debug-setup done → E2E 검증 여전히 최소 수준** — JavaScriptBridge·`playwright-screenshot.yml` 구현 완료됨에도 `smoke.spec.js`는 canvas 렌더만 확인. WS 연결·OAuth 플로우·매치 시작 등 핵심 사용자 경로 미검증 상태.
5. **스텁 기반 통합 테스트 구조적 한계 지속** — `e2e-flow.test.js`가 PostgreSQL·Redis·Gemini를 전량 Module stub으로 대체. 실 인프라 회귀 감지 불가. `redis-memory-server`(devDependency 설치됨) 미활용 20회 연속.

### 개선 제안 (우선순위 포함)

| 우선순위 | 작업 | 상태 | 규모 |
|----------|------|------|------|
| **P0-2** | `.github/workflows/server-test.yml` — `server-test-ci` spec 기준 구현 | ⏳ **needs-review — hyeonseok 착수 승인 대기 (20회 연속)** | ~30줄 |
| **P1-4+P1-5** | `test/oracle-cooldown.test.js` + `test/leaderboard.test.js` — `server-unit-tests-expansion` spec 기준 구현 | 🟡 **queued — runner 착수 대기** | ~100줄 합계 |
| **P1-1** | 실 WebSocket 통합 테스트 — `ws` 클라이언트 + `redis-memory-server` 활용 | 🔲 미착수 (라이브러리 기설치, 장벽 낮음) | ~100줄 |
| **P1-2** | `pg-mem` 마이그레이션 스모크 테스트 — 7개 SQL 순서 적용 검증 | 🔲 미착수 | ~60줄 |
| **P2-1** | Playwright E2E 확장 — WS 연결·OAuth·매치 플로우 검증 (JavaScriptBridge 준비됨) | 🔲 spec 있음, smoke 수준 구현만 존재 | TBD |

**20회차 진단**: 19회차 대비 신규 버그(redis-state.js·matchmaker.js 스탯 누락) 발견·수정이 테스트 실행 중 이루어짐 — CI가 없는 구조에서 이런 버그가 수동 점검 전까지 라이브로 유지될 수 있는 리스크를 재확인. server-unit-tests-expansion spec 완비 후 queued 전환·runner 대기 상태로 진전. P0-2 CI 워크플로는 hyeonseok 명시적 승인 없이는 진전이 없는 교착 구조 — 승인 or 보류 결정을 명시적으로 받을 필요 있음.
...
| 부하 — 32명 동시 | `test/load-32players.test.js` | ✅ p99 < 1,000ms | 실측 p99 ≈ 7.3ms |
| 비용 검증 — Gemini | `test/gemini-cost.test.js` | ✅ $0.005/게임 한도 내 | 정적 계산 ($0.001416 실측) |
| CI 서버 테스트 워크플로 | `.github/workflows/` | ❌ 미포함 | **P0-2 — 18회차 연속** (spec 완비, hyeonseok 착수 승인 대기) |
| oracle-cooldown 단위 테스트 | `test/oracle-cooldown.test.js` | ❌ 부재 | **P1-4** — spec 테스트 명세 완비, DevourerKing 미착수 |
| oracle-ranking-leaderboard 단위 테스트 | `test/leaderboard.test.js` | ❌ 부재 | **P1-5** — spec 테스트 명세 완비, DevourerKing 미착수 |
| 실 WebSocket 통합 테스트 | — | ❌ 부재 | P1-1 미착수 |
| DB 마이그레이션 스모크 테스트 | `migrations/` (7개) | ❌ 부재 | P1-2 미착수 |
| Godot 클라이언트 / Playwright E2E | — | ❌ 부재 | P2-1 — unstaged 11개 무검증 지속 |

### 발견한 문제점 (17회차 대비)

1. **P0-2 CI 워크플로 18회차 연속 미해결** — `server-test-ci` spec 완성, push/PR 트리거·Node.js 20·Redis 7·PostgreSQL 15·AC 5개 명세 완비. hyeonseok 착수 승인만 남은 상태로 진전 없음.
2. **P1-4·P1-5 DevourerKing 착수 없음** — oracle-cooldown(7케이스)·leaderboard(8케이스) spec 완비 상태로 핸드오프 준비 완료. 구현 완료 모듈에 테스트 없이 라이브 상태 지속.
3. **Godot unstaged 11파일 무검증 지속** — LeaderboardScreen, SpectateListScreen 포함. 클라이언트 변경이 단 한 건도 테스트 없이 누적.
4. **스텁 기반 통합 테스트 구조적 한계** — PostgreSQL·Redis·Gemini 전량 모킹으로 실 서비스 회귀 감지 불가. `redis-memory-server` 설치됨에도 P1-1·P1-2 미착수 18회차 지속.
5. **Jest 미활용** — devDependency로 설치(^30.3.0)되어 있으나 설정 없음. 기존 커스텀 harness 6개와 병행 도입 여지 있음.

### 개선 제안 (우선순위 포함)

| 우선순위 | 작업 | 상태 | 규모 |
|----------|------|------|------|
| **P0-2** | `.github/workflows/server-test.yml` — `server-test-ci` spec 기준 구현 | ⏳ **hyeonseok 착수 승인 대기 (18회 연속)** | ~30줄 |
| **P1-4** | `test/oracle-cooldown.test.js` — Redis TTL 60s mock, HTTP 429, matchId 격리 | 🔴 spec 테스트 명세 완비 — DevourerKing 즉시 착수 가능 | ~50줄 |
| **P1-5** | `test/leaderboard.test.js` — tiebreak, NULL, limit, displayName fallback | 🔴 spec 테스트 명세 완비 — DevourerKing 즉시 착수 가능 | ~50줄 |
| **P1-1** | 실 WebSocket 통합 테스트 — `ws` 클라이언트 + `redis-memory-server` 활용 | 🔲 미착수 (라이브러리 기설치) | ~100줄 |
| **P1-2** | `pg-mem` 마이그레이션 스모크 테스트 — 7개 SQL 순서 적용 검증 | 🔲 미착수 | ~60줄 |
| **P2-1** | Playwright E2E — `playwright-debug-setup` spec 기준 구현 | 🔲 spec 있음, 미구현 | TBD |

**18회차 진단**: 17회차 대비 테스트 구조 변화 없음. 서버 단위 6개 통과 유지. P0-2는 18회차 연속 착수 보류 — 판단이 필요한 상황이라면 CI 없이 라이브 배포를 계속하는 리스크를 hyeonseok와 명시적으로 확인할 필요 있음. P1-1에 필요한 `redis-memory-server`가 이미 설치된 점은 착수 장벽이 낮음을 의미하므로 DevourerKing 선착수 권장.

---

## 2026-04-13 점검 17회차 (태연 스케줄 점검 — oracle-battleroyale 테스트 전략)
...
| 부하 — 32명 동시 | `test/load-32players.test.js` | ✅ p99 < 1,000ms | 실측 p99 ≈ 7.3ms |
| 비용 검증 — Gemini | `test/gemini-cost.test.js` | ✅ $0.005/게임 한도 내 | 정적 계산 ($0.001416 실측) |
| CI 서버 테스트 워크플로 | `.github/workflows/` | ❌ 미포함 | **P0-2 — 17회차 연속** (spec 생성됨, 착수 승인 대기) |
| oracle-cooldown 단위 테스트 | `test/oracle-cooldown.test.js` | ❌ 부재 | **P1-4** — spec 테스트 명세 완비, DevourerKing 미착수 |
| oracle-ranking-leaderboard 단위 테스트 | `test/leaderboard.test.js` | ❌ 부재 | **P1-5** — spec 테스트 명세 완비, DevourerKing 미착수 |
| 실 WebSocket 통합 테스트 | — | ❌ 부재 | P1-1 미착수 |
| DB 마이그레이션 스모크 테스트 | `migrations/` (9개) | ❌ 부재 | P1-2 미착수 |
...
1. **P0-2 CI 워크플로 17회차 연속 미해결** — `server-test-ci` spec은 완성 상태. push/PR 트리거, Node.js 20, Redis 7, PostgreSQL 15, AC 5개 명세 완비. hyeonseok 착수 승인만 남은 상태에서 진전 없음.
2. **P1-4·P1-5 DevourerKing 착수 미확인** — oracle-cooldown spec (7케이스), oracle-ranking-leaderboard spec (8케이스) 모두 작성 완료. 구현 완료 모듈에 테스트 없이 라이브 상태 지속.
3. **Godot unstaged 11파일 무검증 지속** — 수정 7개 + 신규 4개 (LeaderboardScreen, SpectateListScreen). 클라이언트 변경이 테스트 없이 누적.
4. **스텁 기반 통합 테스트 구조적 한계 지속** — PostgreSQL, Redis, Gemini 전량 모킹. 실 서비스 회귀 감지 불가. P1-1·P1-2 미착수.

### 개선 제안 (우선순위 포함)

| 우선순위 | 작업 | 상태 | 규모 |
|----------|------|------|------|
| **P0-2** | `.github/workflows/server-test.yml` — `server-test-ci` spec 기준 구현 | ⏳ **hyeonseok 착수 승인 대기 (17회 연속)** | ~30줄 |
| **P1-4** | `test/oracle-cooldown.test.js` — Redis TTL 60s mock, HTTP 429, matchId 격리 | 🔴 spec 테스트 명세 완비 — DevourerKing 즉시 착수 가능 | ~50줄 |
| **P1-5** | `test/leaderboard.test.js` — tiebreak, NULL, limit, displayName fallback | 🔴 spec 테스트 명세 완비 — DevourerKing 즉시 착수 가능 | ~50줄 |
...
**17회차 진단**: 16회차 대비 테스트 구조 변화 없음. 서버 단위 6개 모두 통과 상태 유지. P0-2는 `server-test-ci` spec이 완성 상태임에도 17회차 연속 착수 보류 — hyeonseok 의사결정 재요청 필요. P1-4·P1-5 DevourerKing 핸드오프 문서 완비 상태 → 착수 확인이 필요한 시점.
